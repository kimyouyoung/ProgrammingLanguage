    ...
    (type-case Value*Store vs
        [v*s (value store)
             (type-case BFAE-Value value
                              [exprV (expr ds st value-box)
                                     (if (not (unbox value-box))
                                         (local [(define value (strict (interp expr ds)))]
                                           (begin (set-box! value-box value)
                                                         value))
                                         (v*s (unbox value-box) store))]
                              [else vs])]))

; strict-value
(define (strict-value v)
    (type-case BFAE-Value v
        [exprV (expr ds st v-box)
                     (if (not (unbox v-box))
                          (local [(define val (interp expr ds st))
                                  (define v (strict-value (v*s-value val)))]
